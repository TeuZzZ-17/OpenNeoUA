#ifndef LOCALE_STRID_H
#define LOCALE_STRID_H


namespace Locale {

enum LOCALE_BLOCK
{
    LBL_COMMON      = 0,
    LBL_HUDTXT      = 30,
    LBL_WINNAMES    = 50,
    LBL_TITLES      = 80,
    LBL_BRIEF       = 150,
    LBL_FEEDBACK    = 200,
    LBL_DIALOGS     = 300,
    LBL_NETDLG      = 400,
    LBL_INPUTS      = 500,
    LBL_YOUPLAY     = 600,
    LBL_GLOBMAP     = 640,
    LBL_WAITINGFOR  = 651,
    LBL_ADVICE      = 700,
    LBL_HELP        = 750,
    LBL_TIPS        = 800,
    LBL_KEYNAME     = 1000,
    LBL_VEHICLES    = 1200,
    LBL_BUILDINGS   = 1500,
    LBL_NETBUILDS   = 1700,
    LBL_LEVELNAMES  = 1800,
    LBL_MISSIONSTXT = 2100,
    LBL_ADVANCED    = 2400,
    LBL_TODS        = 2490, // up to 2510
    LBL_GEM         = 2520
};






// OpenNeoUA-owned strings live in a separate range and are loaded from
// Locale/OpenNeoUA.lng. IDs below this range belong to the original
// Urban Assault LANGUAGE.DLL catalogue.
constexpr int OPENUA_STRING_BASE = 3000;
constexpr int OPENUA_STRING_LIMIT = 4000;

enum OPENUA_STRING
{
    OUA_QUIT_GAME = OPENUA_STRING_BASE,
    OUA_QUIT_GAME_CONFIRM,
    OUA_DATABASE,
    OUA_DATABASE_TOOLTIP,
    OUA_HOST_STATION_AI,
    OUA_SPECTATOR_MODE,
    OUA_HOST_STATION_AI_TOOLTIP,
    OUA_SPECTATOR_MODE_TOOLTIP,
    OUA_RETURN_SPECTATOR_CAMERA,
    OUA_WINDOWED_MODE,
    OUA_WINDOWED_MODE_TOOLTIP,
    OUA_LEFT_SHIFT,

    OUA_PLAY_AS,
    OUA_PLAY_AS_FORMAT,
    OUA_UNKNOWN,

    OUA_ATMOSPHERE,
    OUA_STANDARD,
    OUA_BLENDING,
    OUA_DEFAULT,
    OUA_ADDITIVE,
    OUA_SHARP,
    OUA_MENU_FONT,
    OUA_DEFAULT_VIEW,
    OUA_COCKPIT,
    OUA_POV,
    OUA_VHS_FILTER,
    OUA_INTRO_MOVIES,
    OUA_FPS_LIMIT,
    OUA_TOGGLE_COCKPIT_CAMERA,
    OUA_SPRINT,

    OUA_RESTORE_DEFAULT_KEYS_TITLE,
    OUA_RESTORE_DEFAULT_KEYS_TEXT,
    OUA_RESTART_REQUIRED_TITLE,
    OUA_RESTART_REQUIRED_TEXT,

    OUA_DB_DETAILS,
    OUA_DB_PREV,
    OUA_DB_BACK,
    OUA_DB_NEXT,
    OUA_DB_UNITS,
    OUA_DB_WEAPONS,
    OUA_DB_BUILDINGS,
    OUA_DB_STATS,
    OUA_DB_MODEL_FORMAT,
    OUA_DB_HP_FORMAT,
    OUA_DB_DEF_FORMAT,
    OUA_DB_WEAPON_FORMAT,
    OUA_DB_ATK_FORMAT,
    OUA_DB_AOE_ATK_FORMAT,
    OUA_DB_PUSH_FORMAT,
    OUA_DB_AOE_PUSH_FORMAT,
    OUA_DB_WEAPON_RECOIL_FORMAT,
    OUA_DB_POWER_FORMAT,
    OUA_DB_GUNS_FORMAT,
    OUA_DB_PUSH_RESISTANCE_FORMAT,
    OUA_DB_PUSH_RESISTANCE_NONE,
    OUA_DB_NONE,
    OUA_DB_SPECIALITIES,
    OUA_DB_NO_IMAGE,
    OUA_DB_NO_ENTRY,
    OUA_DB_VS_HOST,
    OUA_DB_TANKS,
    OUA_DB_VS_PLANES,
    OUA_DB_HELIS,
    OUA_DB_CONQUER,
    OUA_DB_RECON,
    OUA_DB_CLUSTER_WEAPON,
    OUA_DB_CHAIN_WEAPON,
    OUA_DB_ARMOR_PENETRATION,
    OUA_DB_DEBUFF,
    OUA_DB_MULTI_TARGET,
    OUA_DB_SPAWNER,
    OUA_DB_SPAWN_ON_DEATH,
    OUA_DB_MOBILE_POWER_GENERATOR,
    OUA_DB_RADAR_UNIT,
    OUA_DB_MOBILE_GUN_PLATFORM,
    OUA_DB_MIMIC,
    OUA_DB_INVISIBILITY,
    OUA_DB_INVULNERABLE,
    OUA_DB_KAMIKAZE,
    OUA_DB_PROXIMITY_DEFENSE,
    OUA_DB_PROXIMITY_DEFENSE_AT_DEATH,
    OUA_DB_MULTI_WEAPON,
    OUA_DB_STATIC_GUN_SUPPORT,
    OUA_DB_POWER_GENERATOR,
    OUA_DB_DEFENSIVE_GUNS,
    OUA_DB_RADAR_BUILDING,
    OUA_DB_PAGE_FORMAT,
    OUA_DB_NO_PROTOTYPE_FORMAT,

    OUA_GEM_UPGRADE_UNLOCKED,
    OUA_GEM_UPGRADES_UNLOCKED,
    OUA_GEM_NEW_UNIT_UNLOCKED,
    OUA_GEM_NEW_BUILDING_UNLOCKED,
    OUA_GEM_ATTACK_UPGRADE,
    OUA_GEM_ARMOR_UPGRADE,
    OUA_GEM_ENERGY_UPGRADE,
    OUA_GEM_RADAR_UPGRADE,
    OUA_GEM_AVAILABLE_FOR_CONSTRUCTION,
    OUA_GEM_ALREADY_UNLOCKED,
    OUA_GEM_UNLOCKED,
    OUA_GEM_WEAPON_COUNT_UPGRADE,
    OUA_GEM_FIRE_RATE_UPGRADE,
    OUA_GEM_HELI_DAMAGE_UPGRADE,
    OUA_GEM_TANK_DAMAGE_UPGRADE,
    OUA_GEM_FLYER_DAMAGE_UPGRADE,
    OUA_GEM_ROBO_DAMAGE_UPGRADE,

    OUA_NETWORK_SERVER_ADDRESS,
    OUA_NETWORK_ENTER_SERVER_IP,
    OUA_INTERNAL_CANNOT_HAPPEN,
    OUA_SCREENSHOT_SAVED,
    OUA_SCREENSHOTTING_STOPPED,
    OUA_SCREENSHOTTING_STARTED,
    OUA_REPLAY_RECORDING_STOPPED,
    OUA_REPLAY_RECORDING_STARTED,
    OUA_WAITING_FOR_HOST,

    // OpenNeoUA virtual UI style. Appended to preserve all existing string IDs.
    OUA_INTERFACE_STYLE,
    OUA_RETRO,
    OUA_SMOOTH,
    OUA_RETRO_INTERFACE,

    // Appended to preserve all existing OpenNeoUA string IDs.
    OUA_RENDER_DISTANCE,

    // Legacy advanced-graphics title/control IDs. Preserved for compatibility.
    OUA_ATMOSPHERE_VISIBILITY,
    OUA_VISUAL_FILTER_STRENGTH,
    OUA_ATMOSPHERE_STRENGTH,
    OUA_EXPOSURE,
    OUA_CONTRAST,
    OUA_SATURATION,
    OUA_VIGNETTE,
    OUA_FOG_START,
    OUA_FOG_LENGTH,
    OUA_FOG_STRENGTH,
    OUA_DARK_START,
    OUA_DARK_LENGTH,
    OUA_DARK_STRENGTH,
    OUA_SKY_HEIGHT,
    OUA_VHS_STRENGTH,
    OUA_RESET,
    OUA_VISUAL_FILTER,
    OUA_SAVE_SETTINGS,
    OUA_WORLD_UI_MAX_DISTANCE,
    OUA_MORE_OPTIONS,

    // Manual weapon selection binding. Appended to preserve existing IDs.
    OUA_SWITCH_WEAPON,

    // Remappable action for placing a personal marker on the tactical map.
    OUA_PLACE_MAP_MARKER,

    // Input-remapping conflict confirmation. Appended to preserve existing IDs.
    OUA_KEY_CONFLICT_FORMAT,
    OUA_KEY_CONFLICT_REASSIGN,

    // Contextual zoom labels shared by the tactical map, UFO Spy Camera and
    // Spectator Follow. Appended to preserve every existing OpenNeoUA string ID.
    OUA_ZOOM_IN,
    OUA_ZOOM_OUT,

    // Briefing Play As visibility toggle. Appended to preserve existing IDs.
    OUA_ENABLE_PLAY_AS,

    // Global ambient loop volume exposed in the main Options page.
    OUA_AMBIENT_VOLUME,

    // Remappable homing primary-target cycling. Appended to preserve existing IDs.
    OUA_CYCLE_TARGET,

    // Remappable contextual Alternative View. Appended to preserve existing IDs.
    OUA_ALTERNATIVE_VIEW,

    // Fixed secondary-input labels shown dynamically in Input Settings.
    OUA_LEFT_MOUSE,
    OUA_RIGHT_MOUSE,
    OUA_MOUSE_WHEEL_UP,
    OUA_MOUSE_WHEEL_DOWN,

    // Remappable player-only toggle for the UFO Spy UI. Appended to preserve existing IDs.
    OUA_TOGGLE_UFO_SPY_UI,

    // Extended vanilla combat-class labels. Appended to preserve existing IDs.
    OUA_VS_PLANE,
    OUA_VS_GLIDER,
    OUA_VS_ZEPPELIN,
    OUA_VS_UFO,
    OUA_VS_CAR,
    OUA_VS_GUN,

    // GEM notifications for fine-grained Weapon energy upgrades.
    OUA_GEM_PLANE_DAMAGE_UPGRADE,
    OUA_GEM_GLIDER_DAMAGE_UPGRADE,
    OUA_GEM_ZEPPELIN_DAMAGE_UPGRADE,
    OUA_GEM_UFO_DAMAGE_UPGRADE,
    OUA_GEM_CAR_DAMAGE_UPGRADE,
    OUA_GEM_GUN_DAMAGE_UPGRADE,

    // Cruiser extension. Appended to preserve every existing OpenNeoUA string ID.
    OUA_VS_CRUISER,
    OUA_GEM_CRUISER_DAMAGE_UPGRADE,

    // Genesis per-Vehicle production limit upgrade notification.
    OUA_GEM_MAX_UNITS_UPGRADE,

    // Visual-only toggle for the vanilla RAND map boundary walls.
    OUA_HIDE_MAP_BORDER_WALLS,

    // Mission briefing/debrief controls and mission-local Plasma statistic.
    OUA_QUIT_MISSION,
    OUA_REPLAY_BRIEFING,
    OUA_RESTART_MISSION,
    OUA_COMPLETE_MISSION,
    OUA_PLASMA_OBTAINED,

    // Weapon HUD magazine/salve label owned by OpenNeoUA.
    OUA_HUD_MAG,

    // Advanced Graphics Settings runtime limits. Keep the retired Render Sectors
    // slot reserved so every later OpenNeoUA language ID remains stable.
    OUA_PARTICLE_LIMIT,
    OUA_RESERVED_RENDER_SECTORS,

    // Data-driven graphics preset selector. Appended to preserve existing IDs.
    OUA_GRAPHIC_PRESET,
    OUA_CUSTOM,

    // Advanced graphics page title. Appended so older OpenNeoUA.lng files
    // cannot override the new title through the legacy title ID.
    OUA_ADVANCED_GRAPHICS_SETTINGS,

    // Player-centered sky-dome radius / world horizon control.
    OUA_SKY_HORIZON_DISTANCE,

    OUA_STRING_MAX
};

enum CMN
{
    CMN_YES         = 0,
    CMN_NO          = 1,
    CMN_OK          = 2,
    CMN_CANCEL      = 3,
    CMN_LOAD        = 4,
    CMN_SAVE        = 5,
    CMN_WELCOME     = 6,
    CMN_EXITMISN    = 7,
    CMN_RESTART     = 8,
    CMN_CONTINUE    = 9,
    CMN_UNITDESTR   = 10,
    CMN_HSDESTR     = 11,
    CMN_MISNCONT    = 12,
    CMN_RESETDEF    = 13,
    CMN_PAUSED      = 14,
    CMN_FONT        = 15,
    CMN_SMALLFONT   = 16,
    CMN_SYSTEXTINPT = 17,
    CMN_BOMBNAME    = 18,
    CMN_WAVENAME    = 19,
    CMN_HELP        = 20,
    CMN_GAMENAME    = 21,
    CMN_QUITGAME    = 22,

    CMN_MAX
};

enum HUDSTR
{
    HUDSTR_DMG  = 2,
    HUDSTR_RLD  = 3,
    HUDSTR_OK   = 4,
    HUDSTR_EGY  = 5,
    HUDSTR_AMR  = 6,

    HUDSTR_MAX
};

enum GEMSTR
{
    GEMSTR_UPGRADE_UNLOCKED = 0,
    GEMSTR_UPGRADES_UNLOCKED,
    GEMSTR_NEW_UNIT_UNLOCKED,
    GEMSTR_NEW_BUILDING_UNLOCKED,
    GEMSTR_ATTACK_UPGRADE,
    GEMSTR_ARMOR_UPGRADE,
    GEMSTR_ENERGY_UPGRADE,
    GEMSTR_RADAR_UPGRADE,
    GEMSTR_AVAILABLE_FOR_CONSTRUCTION,
    GEMSTR_ALREADY_UNLOCKED,
    GEMSTR_UNLOCKED,
    GEMSTR_WEAPON_COUNT_UPGRADE,
    GEMSTR_FIRE_RATE_UPGRADE,
    GEMSTR_HELI_DAMAGE_UPGRADE,
    GEMSTR_TANK_DAMAGE_UPGRADE,
    GEMSTR_FLYER_DAMAGE_UPGRADE,
    GEMSTR_ROBO_DAMAGE_UPGRADE,

    GEMSTR_MAX
};

enum WINNAME
{
    WINNAME_MAP     = 0,
    WINNAME_FINDER  = 1,
    WINNAME_LOG     = 2,
    WINNAME_EXIT    = 3,

    WINNAME_MAX
};

enum TITLE
{
    TITLE_GAME     = 0,
    TITLE_NETWORK  = 1,
    TITLE_INPUT    = 3,
    TITLE_SETTINGS = 4,
    TITLE_PROFILE  = 5,
    TITLE_LOCALE   = 6,
    TITLE_HELP     = 7,
    TITLE_QUIT     = 8,
    TITLE_DATABASE = 9,

    TITLE_MAX
};

enum BRIEF
{
    BRIEF_YOU       = 0,
    BRIEF_PRIM      = 1,
    BRIEF_TECH      = 2,
    BRIEF_ENMYHS    = 3,
    BRIEF_ENMYFORCE = 4,
    BRIEF_OWNFORCES = 5,
    BRIEF_GATE      = 6,
    BRIEF_KEYSECT   = 7,
    BRIEF_TECH2     = 8,
    BRIEF_GATE2     = 9,
    BRIEF_PS        = 10,

    BRIEF_MAX
};

enum FEEDBACK
{
    FEEDBACK_SECTCONQ   = 2,
    FEEDBACK_FOUNDEHS   = 6,
    FEEDBACK_HSDESTR    = 11,
    FEEDBACK_OWNHSDESTR = 12,
    FEEDBACK_ENMYNEARHS = 13,
    FEEDBACK_REQSUP     = 18,
    FEEDBACK_TECHUP     = 21,
    FEEDBACK_ESECTENTER = 22,
    FEEDBACK_GATEOPEN   = 23,
    FEEDBACK_GATECLOSE  = 24,
    FEEDBACK_TECHDOWN   = 29,
    FEEDBACK_FLAKDESTR  = 31,
    FEEDBACK_RADARDESTR = 32,
    FEEDBACK_ATKPS      = 33,
    FEEDBACK_ATKFLAK    = 34,
    FEEDBACK_ATKRADAR   = 35,
    FEEDBACK_HSENERGY   = 40,
    FEEDBACK_BOMBACT    = 50,
    FEEDBACK_BOMBTRIG   = 51,
    FEEDBACK_BOMBFROZ   = 52,
    FEEDBACK_BOMBDEACT  = 53,
    FEEDBACK_WAVEACT    = 54,
    FEEDBACK_WAVETRIG   = 55,
    FEEDBACK_WAVEFROZ   = 56,
    FEEDBACK_WAVEDEACT  = 57,
    FEEDBACK_GATEFULL   = 58,
    FEEDBACK_GAMESAVED  = 59,

    FEEDBACK_MAX
};

enum DIALOGS
{
    DLG_I_JOYSTICK  = 5,
    DLG_I_FF        = 6,
    DLG_I_DELETE    = 7,
    DLG_I_UNK       = 8,
    DLG_I_TITLE     = 9,
    DLG_I_TXT2      = 10,
    DLG_I_TXT3      = 11,
    DLG_I_TXT4      = 12,

    DLG_S_FXVOL     = 21,
    DLG_S_INVERT    = 23,
    DLG_S_CDVOL     = 24,
    DLG_S_ENMYINDIC = 25,
    DLG_S_ENCDAUD   = 26,
    DLG_S_TITLE     = 27,
    DLG_S_TXT2      = 28,
    DLG_S_TXT3      = 29,
    DLG_S_TXT4      = 30,
    DLG_S_HOSTSTATIONAI = 31,
    DLG_S_SPECTATORMODE = 32,
    DLG_S_RES       = 40,
    DLG_S_RESCHANGE = 41,
    DLG_S_RESWARN   = 42,
    DLG_S_DESTRFX   = 43,
    DLG_S_FARVIEW   = 44,
    DLG_S_SKY       = 45,
    DLG_S_SWMOUSE   = 50,
    DLG_S_SEL3D     = 52,

    DLG_P_LOAD      = 60,
    DLG_P_SAVE      = 61,
    DLG_P_DELETE    = 62,
    DLG_P_NEW       = 63,
    DLG_P_UNNAMED   = 66,
    DLG_P_TITLE     = 67,
    DLG_P_TXT2      = 68,
    DLG_P_TXT3      = 69,
    DLG_P_TXT4      = 70,

    DLG_L_TITLE     = 95,
    DLG_L_TXT2      = 96,
    DLG_L_TXT3      = 97,
    DLG_L_TXT4      = 98,

    DLG_MAX
};

enum NETDLG
{
    NETDLG_NEXT     = 0,
    NETDLG_BACK     = 1,
    NETDLG_NEW      = 2,
    NETDLG_CANCEL   = 3,
    NETDLG_SEND     = 5,
    NETDLG_JOIN     = 6,
    NETDLG_START    = 7,
    NETDLG_READY    = 9,
    NETDLG_SELPROV  = 10,
    NETDLG_SELSESS  = 11,
    NETDLG_SELLVL   = 12,
    NETDLG_ENTERPL  = 13,
    NETDLG_STARTOR  = 14,
    NETDLG_WAITOR   = 15,
    NETDLG_CONNECT  = 21,
    NETDLG_SELRACE  = 24,
    NETDLG_TXT2     = 25,
    NETDLG_TXT3     = 26,
    NETDLG_TXT5     = 28,
    NETDLG_TXT6     = 29,
    NETDLG_TXT8     = 31,
    NETDLG_TXT9     = 32,
    NETDLG_TXT11    = 34,
    NETDLG_TXT12    = 35,
    NETDLG_TXT14    = 37,
    NETDLG_TXT15    = 38,
    NETDLG_TXT17    = 40,
    NETDLG_TXT18    = 41,

    NETDLG_MAX
};

enum INPUTS
{
    INPUTS_DRIVEDIR     = 0,
    INPUTS_DRIVESPD     = 1,
    INPUTS_FLYHGHT      = 2,
    INPUTS_FLYSPD       = 3,
    INPUTS_FLYDIR       = 4,
    INPUTS_STOP         = 5,
    INPUTS_FIRE         = 6,
    INPUTS_FIREVW       = 7,
    INPUTS_FIREGUN      = 8,
    INPUTS_GUNHGHT      = 11,
    INPUTS_ORDER        = 13,
    INPUTS_FIGHT        = 14,
    INPUTS_NEW          = 15,
    INPUTS_ADD          = 16,
    INPUTS_CONTROL      = 17,
    INPUTS_AUTOPILOT    = 20,
    INPUTS_MAP          = 21,
    INPUTS_FINDER       = 22,
    INPUTS_LANDSCAPE    = 23,
    INPUTS_OWNER        = 24,
    INPUTS_HEIGHT       = 25,
    INPUTS_LOCKVW       = 27,
    INPUTS_ZOOMIN       = 29,
    INPUTS_ZOOMOUT      = 30,
    INPUTS_MAPMINI      = 31,
    INPUTS_NEXTCOM      = 32,
    INPUTS_TOROBO       = 33,
    INPUTS_NEXTUNIT     = 34,
    INPUTS_TOCOMM       = 35,
    INPUTS_QUIT         = 36,
    INPUTS_LOGWIN       = 38,
    INPUTS_HEADUPDISP   = 41,
    INPUTS_JUMPLASTMSNG = 43,
    INPUTS_PAUSE        = 44,
    INPUTS_MSGTOALL     = 52,
    INPUTS_AGGR1        = 53,
    INPUTS_AGGR2        = 54,
    INPUTS_AGGR3        = 55,
    INPUTS_AGGR4        = 56,
    INPUTS_AGGR5        = 57,
    INPUTS_SELWAYPT     = 58,
    INPUTS_HELP         = 59,
    INPUTS_TOLASTOCCUP  = 60,
    INPUTS_MAKECOMM     = 61,
    INPUTS_ANALYZER     = 62,

    INPUTS_MAX
};

enum GLOBMAP
{
    GLOBMAP_REWIND  = 0,
    GLOBMAP_STEPFWD = 1,
    GLOBMAP_LOAD    = 2,
    GLOBMAP_START   = 3,
    GLOBMAP_GOBACK  = 4,

    GLOBMAP_MAX
};

enum TIP
{
    TIP_NONE        = 0,

    TIP_GUI_GIVEORDERS      = 1,
    TIP_GUI_GOINTOGUNS      = 2,
    TIP_GUI_NEWSQUAD        = 3,
    TIP_GUI_ADDSQUAD        = 4,
    TIP_GUI_TAKECONTROL     = 5,
    TIP_GUI_BUILD           = 6,
    TIP_GUI_TELEPORT        = 7,
    TIP_GUI_MAPWND          = 8,
    TIP_GUI_FINDERWND       = 9,
    TIP_GUI_MSGWND          = 10,
    TIP_GUI_ENERGYWND       = 11,

    TIP_GUI_AGGR_0          = 12,
    TIP_GUI_AGGR_1          = 13,
    TIP_GUI_AGGR_2          = 14,
    TIP_GUI_AGGR_3          = 15,
    TIP_GUI_AGGR_4          = 16,

    TIP_GUI_GOTOHS          = 17,
    TIP_GUI_GOTOCMDR        = 18,
    TIP_GUI_NEXTUNIT        = 19,
    TIP_GUI_NEXTCMDR        = 20,

    TIP_GUI_PAUSECANCEL     = 21,

    TIP_MAP_DETAIL          = 22,
    TIP_MAP_OWNERS          = 23,
    TIP_MAP_EINFO           = 24,
    TIP_MAP_UNLOCK          = 25,
    TIP_MAP_LOCKVIEWER      = 26,
    TIP_MAP_LOCKSQUAD       = 27,
    TIP_MAP_ZOOMOUT         = 28,
    TIP_MAP_ZOOMIN          = 29,
    TIP_MAP_SIZE            = 30,

    TIP_FINDER_WAITORDER    = 31,
    TIP_FINDER_FIGHTING     = 32,
    TIP_FINDER_MOVE         = 33,
    TIP_FINDER_ESCAPE       = 34,

    TIP_ENERGY_RELOAD       = 35,
    TIP_ENERGY_MAIN         = 36,
    TIP_ENERGY_CREATION     = 37,
    TIP_ENERGY_BUILDING     = 38,
    TIP_ENERGY_TELEPORT     = 39,

    TIP_SEL_ANOTHERACT      = 40,
    TIP_SEL_DEFENSEGUN      = 41,
    TIP_SEL_VEHICLETYPE     = 42,
    TIP_SEL_BUILDTYPE       = 43,

    TIP_HUD_SHOWHIDE        = 44,
    TIP_CTRLASTSNDR         = 45,

    TIP_COUNTINSQUAD        = 46,
    TIP_ANALYZER            = 47,
    TIP_ONLINEHELP          = 48,
    TIP_GUI_RETURNSPECTATOR = 49,

    TIP_DO_SELECT           = 64,
    TIP_DO_MOVE             = 65,
    TIP_DO_ATKSECTOR        = 66,
    TIP_DO_ATKVHCL          = 67,
    TIP_DO_NEWSQUAD         = 68,
    TIP_DO_ADDSQUAD         = 69,
    TIP_DO_CONTROL          = 70,
    TIP_DO_BUILD            = 71,
    TIP_DO_TELEPORT         = 72,

    TIP_MENU_EXITDEBRIEF    = 73,
    TIP_MENU_GOTOLOADSAVE   = 74,
    TIP_MENU_SEL3DDEV       = 75,
    TIP_MENU_SENDMSG        = 76,
    TIP_MENU_ALTJOY         = 77,
    TIP_MENU_16BITTEX       = 78,
    TIP_MENU_DRAWPRIMITIVE  = 79,

    TIP_MOUSECAPTURED       = 80,

    TIP_UNITLIMIT           = 81,

    TIP_MENU_WINDOWED       = 85,

    TIP_ERR_NOTARGET        = 96,
    TIP_ERR_NOROOM          = 97,
    TIP_ERR_NOENERGY        = 98,
    TIP_ERR_NOTOWNED        = 99,
    TIP_ERR_BUILD_TOOFAR    = 100,
    TIP_ERR_BUILD_TOOCLOSE  = 101,
    TIP_ERR_BUILD_INPROC    = 102,
    TIP_ERR_UNITSINSECTOR   = 103,
    TIP_ERR_SECTORJAGGY     = 104,
    TIP_ERR_LOCUNKNOWN      = 105,

    TIP_MENU_LVLSELECTMAP   = 106,
    TIP_MENU_GOTRAIN        = 107,
    TIP_MENU_LANGCHOOSE     = 108,
    TIP_MENU_LANGEXIT       = 109,
    TIP_MENU_SELNEWLEVEL    = 110,
    TIP_MENU_SENDTOALL      = 111,
    TIP_MENU_SELRESIST      = 112,
    TIP_MENU_SELGHOR        = 113,
    TIP_MENU_SELMYKO        = 114,
    TIP_MENU_SELTAER        = 115,
    TIP_MENU_READY          = 116,
    TIP_MENU_NOTREADY       = 117,

    TIP_MENU_LOADUSER       = 118,
    TIP_MENU_SAVEUSER       = 119,

    TIP_MENU_INPUTSCREEN    = 120,
    TIP_MENU_VIDEOSCREEN    = 121,
    TIP_MENU_SOUNDSCREEN    = 122,
    TIP_MENU_PROFILESCREEN  = 123,
    TIP_MENU_LANGSCREEN     = 124,

    TIP_MENU_BRIEFCTRL      = 125,
    TIP_MENU_BRIEFSTOP      = 126,
    TIP_MENU_LOADLAST       = 127,
    TIP_MENU_BRIEFFAST      = 128,
    TIP_MENU_BRIEFRESET     = 129,
    TIP_MENU_PLAYLEVEL      = 130,
    TIP_MENU_QUIT           = 131,
    TIP_MENU_PRESSADDS      = 132,
    TIP_MENU_USEKEY         = 133,
    TIP_MENU_SOUNDLEFT      = 134,
    TIP_MENU_SOUNDRIGHT     = 135,
    TIP_MENU_NETSCREEN      = 136,
    TIP_MENU_HELP           = 137,
    TIP_MENU_TOMAIN         = 138,
    TIP_MENU_BACKTOLVLSEL   = 139,
    TIP_MENU_NETCLOSE       = 140,
    TIP_MENU_NETHELP        = 141,
    TIP_MENU_NETSELPROV     = 142,
    TIP_MENU_NETPLNAME      = 143,
    TIP_MENU_NETSELLVL      = 144,
    TIP_MENU_NETSELSESS     = 145,
    TIP_MENU_NETSTART       = 146,
    TIP_MENU_NETBACKPROV    = 147,
    TIP_MENU_NETBACKPLAYER  = 148,
    TIP_MENU_NETBACKSESS    = 149,

    TIP_CONF_HOSTSTATIONAI  = 150,
    TIP_MENU_151            = 151,
    TIP_MENU_152            = 152,
    TIP_MENU_153            = 153,
    TIP_MENU_154            = 154,
    TIP_MENU_155            = 155,
    TIP_MENU_156            = 156,
    TIP_MENU_157            = 157,
    TIP_MENU_158            = 158,
    TIP_MENU_159            = 159,
    TIP_MENU_160            = 160,
    TIP_MENU_161            = 161,
    TIP_MENU_162            = 162,
    TIP_MENU_163            = 163,
    TIP_MENU_164            = 164,
    TIP_MENU_165            = 165,
    TIP_MENU_166            = 166,
    TIP_MENU_167            = 167,

    TIP_PROFILE_DEL         = 168,
    TIP_PFOFILE_NEW         = 169,
    TIP_PFOFILE_LOADTHIS    = 170,
    TIP_PFOFILE_SAVETHIS    = 171,
    TIP_PFOFILE_DELTHIS     = 172,
    TIP_PFOFILE_NEWTHIS     = 173,
    TIP_PFOFILE_CLOSE       = 174,
    TIP_PROFILE_CANCELLOAD  = 175,
    TIP_PROFILE_CANCELSAVE  = 176,
    TIP_PROFILE_CANCELDEL   = 177,
    TIP_PROFILE_CANCELNEW   = 178,

    TIP_CONF_CLOSE          = 179,
    TIP_CONF_APPLY          = 180,
    TIP_CONF_RESOLUTION     = 181,
    TIP_CONF_FXSND          = 182,
    TIP_CONF_CHANNELS       = 183,
    TIP_CONF_FARDEPTH       = 184,
    TIP_CONF_SKY            = 185,
    TIP_CONF_FILTERING      = 186,
    TIP_CONF_MUSIC          = 187,
    TIP_CONF_HWSWMOUSE      = 188,
    TIP_CONF_ENEMYSIGNS     = 189,
    TIP_CONF_FXVOLUME       = 190,
    TIP_CONF_MUSICVOLUME    = 191,
    TIP_CONF_EXPLODES       = 192,

    TIP_INPUT_APPLY         = 193,
    TIP_INPUT_CANCEL        = 194,
    TIP_INPUT_RESET         = 195,
    TIP_INPUT_USEJOY        = 196,
    TIP_INPUT_FFJOY         = 197,
    TIP_INPUT_REMOVEKEY     = 198,
    TIP_CONF_SPECTATORMODE  = 199,

    TIP_MAX
};

enum ADVICE
{
    ADVICE_ESCAPETOPS   = 0,
    ADVICE_SCREWED      = 1,
    ADVICE_DEFENDHS     = 2,
    ADVICE_SENDFORCES   = 3,
    ADVICE_CONQPOWEREFF = 4,
    ADVICE_BEAMTOSTRONG = 5,
    ADVICE_CONQPS       = 6,
    ADVICE_BUILDPS      = 7,
    ADVICE_CONQTECH     = 8,
    ADVICE_CONQGATE     = 9,
    ADVICE_CONQKEYGATE  = 10,
    ADVICE_COLLECTTOPS  = 11,
    ADVICE_COLLECTTOHS  = 12,
    ADVICE_EXPLORE      = 13,
    ADVICE_CONQLAND     = 14,
    ADVICE_FORCESTOBEAM = 15,
    ADVICE_NOPROBLEM    = 16,

    ADVICE_MAX
};

enum HELPS
{
    HELP_MAIN       = 0,
    HELP_NETSELPROV = 3,
    HELP_NETNAME    = 4,
    HELP_NETSELSESS = 5,
    HELP_NETSELLVL  = 6,
    HELP_NETSTRTSCR = 7,
    HELP_SAVEGAME   = 8,
    HELP_INPUTCONF  = 9,
    HELP_SETTINGS   = 10,
    HELP_LOCALE     = 11,
    HELP_MAPWND     = 14,
    HELP_FINDERWND  = 15,
    HELP_GAMEMENU   = 16,
    HELP_GAMEPLAY   = 17,

    HELP_MAX
};

enum KEYNAME
{
    KEYNAME_NOP = 0,
    KEYNAME_ESC = 1,
    KEYNAME_SPACE = 2,
    KEYNAME_UP = 3,
    KEYNAME_DOWN = 4,
    KEYNAME_LEFT = 5,
    KEYNAME_RIGHT = 6,
    KEYNAME_F1 = 7,
    KEYNAME_F2 = 8,
    KEYNAME_F3 = 9,
    KEYNAME_F4 = 10,
    KEYNAME_F5 = 11,
    KEYNAME_F6 = 12,
    KEYNAME_F7 = 13,
    KEYNAME_F8 = 14,
    KEYNAME_F9 = 15,
    KEYNAME_F10 = 16,
    KEYNAME_F11 = 17,
    KEYNAME_F12 = 18,
    KEYNAME_BACK = 19,
    KEYNAME_TAB = 20,
    KEYNAME_CLEAR = 21,
    KEYNAME_RETURN = 22,
    KEYNAME_CTRL = 23,
    KEYNAME_SHIFT = 24,
    KEYNAME_ALT = 25,
    KEYNAME_PAUSE = 26,
    KEYNAME_PGUP = 27,
    KEYNAME_PGDOWN = 28,
    KEYNAME_END = 29,
    KEYNAME_HOME = 30,
    KEYNAME_SELECT = 31,
    KEYNAME_EXEC = 32,
    KEYNAME_PRINT = 33,
    KEYNAME_INS = 34,
    KEYNAME_DEL = 35,
    KEYNAME_HELP = 36,
    KEYNAME_1 = 37,
    KEYNAME_2 = 38,
    KEYNAME_3 = 39,
    KEYNAME_4 = 40,
    KEYNAME_5 = 41,
    KEYNAME_6 = 42,
    KEYNAME_7 = 43,
    KEYNAME_8 = 44,
    KEYNAME_9 = 45,
    KEYNAME_0 = 46,
    KEYNAME_A = 47,
    KEYNAME_B = 48,
    KEYNAME_C = 49,
    KEYNAME_D = 50,
    KEYNAME_E = 51,
    KEYNAME_F = 52,
    KEYNAME_G = 53,
    KEYNAME_H = 54,
    KEYNAME_I = 55,
    KEYNAME_J = 56,
    KEYNAME_K = 57,
    KEYNAME_L = 58,
    KEYNAME_M = 59,
    KEYNAME_N = 60,
    KEYNAME_O = 61,
    KEYNAME_P = 62,
    KEYNAME_Q = 63,
    KEYNAME_R = 64,
    KEYNAME_S = 65,
    KEYNAME_T = 66,
    KEYNAME_U = 67,
    KEYNAME_V = 68,
    KEYNAME_W = 69,
    KEYNAME_X = 70,
    KEYNAME_Y = 71,
    KEYNAME_Z = 72,
    KEYNAME_NUM_0 = 73,
    KEYNAME_NUM_1 = 74,
    KEYNAME_NUM_2 = 75,
    KEYNAME_NUM_3 = 76,
    KEYNAME_NUM_4 = 77,
    KEYNAME_NUM_5 = 78,
    KEYNAME_NUM_6 = 79,
    KEYNAME_NUM_7 = 80,
    KEYNAME_NUM_8 = 81,
    KEYNAME_NUM_9 = 82,
    KEYNAME_MUL = 83,
    KEYNAME_ADD = 84,
    KEYNAME_DOT = 85,
    KEYNAME_SUB = 86,
    KEYNAME_ENTER = 87,
    KEYNAME_DIV = 88,
    KEYNAME_EXTRA_1 = 89,
    KEYNAME_EXTRA_2 = 90,
    KEYNAME_EXTRA_3 = 91,
    KEYNAME_EXTRA_4 = 92,
    KEYNAME_EXTRA_5 = 93,
    KEYNAME_EXTRA_6 = 94,
    KEYNAME_EXTRA_7 = 95,
    KEYNAME_EXTRA_8 = 96,
    KEYNAME_EXTRA_9 = 97,
    KEYNAME_EXTRA_10 = 98,
    KEYNAME_EXTRA_11 = 99,
    KEYNAME_EXTRA_12 = 100,
    KEYNAME_EXTRA_13 = 101,
    KEYNAME_EXTRA_14 = 102,
    KEYNAME_EXTRA_15 = 103,
    KEYNAME_EXTRA_16 = 104,
    KEYNAME_EXTRA_17 = 105,
    KEYNAME_EXTRA_18 = 106,

    KEYNAME_MIDDLE_MOUSE = 121,

    KEYNAME_JOYB0 = 123,
    KEYNAME_JOYB1 = 124,
    KEYNAME_JOYB2 = 125,
    KEYNAME_JOYB3 = 126,
    KEYNAME_JOYB4 = 127,
    KEYNAME_JOYB5 = 128,
    KEYNAME_JOYB6 = 129,
    KEYNAME_JOYB7 = 130,
    KEYNAME_LSHIFT = 131,
    KEYNAME_MAX
};

enum ADV
{
    ADV_ERRHEAD = 0,
    ADV_ERRNOSESS = 1,
    ADV_REFRESHSESS = 2,
    ADV_COMPUTER = 3,
    ADV_HASHMISS = 4,
    ADV_HOSTLATENCY = 5,
    ADV_CLILATENCY = 6,
    ADV_NETSTATUS = 7,
    ADV_UNRESPROBLM = 8,
    ADV_NETISOK = 9,
    ADV_RESIST = 11,
    ADV_SULG = 12,
    ADV_MYKO = 13,
    ADV_TAER = 14,
    ADV_BLKSCT = 15,
    ADV_GHOR = 16,
    ADV_NEUTRAL = 17,
    ADV_CONTINUE = 20,
    ADV_GOTOLDSV = 22,
    ADV_WAIT1 = 23,
    ADV_WAIT2 = 24,
    ADV_KICKEDPROBLEM = 25,
    ADV_LVLAUTOFINISH = 26,
    ADV_PLREMOVED = 27,
    ADV_COZNETPROBLEM = 28,
    ADV_NOCONNTOPL = 29,
    ADV_FINISHIFNOT = 30,
    ADV_USE16BIT = 31,
    ADV_ALTJOYMODEL = 33,
    ADV_WANTTOLOAD = 34,
    ADV_WANTSTART = 35,
    ADV_WANTOVERWRITE = 36,
    ADV_YOURIP = 37,
    ADV_BACK = 38,
    ADV_DISCARDPROGRESS = 39,
    ADV_2440 = 40,
    ADV_EXISTSAVEDGAME = 41,
    ADV_NETALONE = 42,
    ADV_WINDOWEDMODE = 45,
    ADV_KILLSFACTION = 50,
    ADV_KILLSBYPLAYER = 51,
    ADV_KILLSALL = 52,
    ADV_SCORE = 53,
    ADV_MISSIONTIME = 54,
    ADV_OVERALLTIME = 55,
    ADV_PLAYTIME = 56,
    ADV_SCOREMISSION = 57,
    ADV_SCOREOVERALL = 58,
    ADV_WEAPONUPG = 59,
    ADV_ARMORUPG = 60,
    ADV_NEWVEHICLE = 61,
    ADV_NEWBUILDING = 62,
    ADV_RADARUPG = 63,
    ADV_COMBINEDUPG = 64,
    ADV_GENERICUPG = 65,
    ADV_LOADINGOBJ = 67,
    ADV_VICTORY = 68,
    ADV_DEFEATED = 69,
    ADV_EXITRAINING = 70,
    ADV_TRIGGERED = 71,
    ADV_SOFTWARE = 72,
    ADV_UNITS = 73,
    ADV_VSROBO = 74,
    ADV_VSTANK = 75,
    ADV_VSPLANE = 76,
    ADV_VSHELI = 77,
    ADV_RECON = 78,
    ADV_CAPTURE = 79,
    ADV_RLYEXIT = 80,
    ADV_RLYSAVE = 81,
    ADV_RLYLOAD = 82,
    ADV_RLYRESTART = 83,
    ADV_RLYHELP = 86,
    ADV_RLYQUITGAME = 87,
    ADV_MAX
};

}

#endif
